...
def setup_db():
    ...
    with get_db() as conn:
        ...
        conn.execute(
            "INSERT INTO fields (id, user_id, name, field_code, area) VALUES (1, 1, 'テスト圃場', 'F001', 1.0)"
        )
        conn.execute(
            "INSERT INTO fields (id, user_id, name, field_code, area) VALUES (2, 1, 'テスト圃場2', 'F002', 2.0)"
        )
        conn.commit()
    ensure_crop_tables()
    ensure_paddy_polygons_table()
