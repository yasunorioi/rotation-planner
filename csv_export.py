...
def export_pesticide_order_csv(
    ...
    Args:
        summary_data: 農薬サマリデータ
            [{"農薬名": str, "必要量": float, "単位": str, "対象作物": str}, ...]
...
def create_csv_export_ui(user_state):
    ...
    # JA職員・管理者向け
    with gr.Accordion("🏢 JA集計（JA職員・管理者のみ）", open=False) as ja_accordion:
...
if __name__ == "__main__":
    ...
    # ほ場CSVエクスポートテスト
    filepath, msg = export_fields_csv(test_user)
    print(f"ほ場CSV: {msg}")
    if filepath:
        print(f"  -> {filepath}")

    # 輪作計画CSVエクスポートテスト
    filepath, msg = export_all_plans_csv(test_user)
    print(f"輪作計画CSV: {msg}")
    if filepath:
        print(f"  -> {filepath}")
