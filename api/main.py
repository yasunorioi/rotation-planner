...
@app.get("/api/crop-families", response_model=Dict[str, str])
...
@app.get("/api/paddy-polygons", response_model=List[PaddyPolygonResponse])
...
@app.get("/api/paddy-polygons/stats")
...
@app.get("/api/paddy-polygons/{polygon_id}", response_model=PaddyPolygonResponse)
...
@app.post("/api/paddy-polygons", response_model=PaddyPolygonResponse, status_code=201)
def create_paddy_polygon(req: PaddyPolygonCreate, current_user: Dict = Depends(get_current_user)):
    ...
    # GeoJSON → 面積自動計算
    geometry_str = _json.dumps(req.geometry)
    try:
...
@app.put("/api/paddy-polygons/{polygon_id}", response_model=PaddyPolygonResponse)
...
@app.delete("/api/paddy-polygons/{polygon_id}")
...
@app.get("/api/crop-polygons", response_model=List[CropPolygonResponse])
def list_crop_polygons(
    ...
    """ほ場・年度別作付けポリゴン一覧"""
    from rotation_planner.common.db_access import CropPolygonRepository, ensure_crop_polygons_table
...
@app.get("/api/crop-polygons/{polygon_id}", response_model=CropPolygonResponse)
...
@app.post("/api/crop-polygons", response_model=CropPolygonResponse, status_code=201)
...
@app.put("/api/crop-polygons/{polygon_id}", response_model=CropPolygonResponse)
...
@app.delete("/api/crop-polygons/{polygon_id}")
...
@app.post("/api/crop-polygons/copy-year")
...
@app.get("/api/aggregation/cross-table")
def get_cross_table(year: int, current_user: Dict = Depends(get_current_user)):
    """クロス集計表JSON（作物×地目）"""
    from rotation_planner.common.db_access import (
        ensure_paddy_polygons_table, ensure_crop_polygons_table,
    )
    from rotation_planner.field.aggregation_service import get_cross_tabulation_for_user
...
@app.get("/api/aggregation/cross-table/csv")
def get_cross_table_csv(year: int, current_user: Dict = Depends(get_current_user)):
    """クロス集計表CSV形式ダウンロード"""
    from rotation_planner.common.db_access import (
        ensure_paddy_polygons_table, ensure_crop_polygons_table,
    )
    from rotation_planner.field.aggregation_service import export_cross_tabulation_csv
...
@app.get("/api/aggregation/subsidy-summary")
def get_subsidy_summary_api(current_user: Dict = Depends(get_current_user)):
    """補助金残年数サマリ"""
    from rotation_planner.common.db_access import ensure_paddy_polygons_table
    from rotation_planner.field.aggregation_service import get_subsidy_summary
...
@app.get("/api/aggregation/land-category")
def get_land_category_for_crop(
    ...
    """単一作付けの地目内訳（デバッグ/確認用）"""
    from rotation_planner.common.db_access import (
        CropPolygonRepository, PaddyPolygonRepository,
        ensure_paddy_polygons_table, ensure_crop_polygons_table,
    )
    from rotation_planner.field.aggregation_service import build_land_breakdown_for_crop
